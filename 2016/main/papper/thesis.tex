  ...
  \section{模型建立}
	...
	\subsection{问题二}
		...
		\subsubsection{基于系统最优分配Braess悖论的判别}
			...
			查阅论文可知Pas和Principcipio在其论文中指出了Braess悖论不发生的两种情况\upcite{pas1997braess}，一种是交通需求量低时，如\eqref{eq:Q_low}，另一种时交通需求量高时\eqref{eq:Q_high}，当交通量符合上述两种情况时，不发生Braess现象，其中$\gamma_n$与$\delta_n$分别为总阻抗函数的常数项系数与一次项系数。
			\begin{equation}
