...
\begin{document}
  ...
  \section{问题分析}
	...
	\subsection{问题二}
		车辆通行时受到多方面因素的影响，我们通过建立模型得出车辆从起点到终点所用的时间，以此来分析开放小区对周边交通的影响。其中所用时间可以分为行驶在路上花费的行程时间，与因为其他因素造成的延误时间。

		\subsubsection{延误时间}
			延误时间\upcite{任福田2003交通工程学}是指，道路上通行所需时间除行走时间外，也受市政道路交通信号灯的影响，如\eqref{eq:delay_func}所示。
			\begin{equation}
			t_d=\frac{0.5T(1-\frac{t_g}{T})}{1-[min(1,x)\cdot{\frac{t_g}{T}}]}
			\label{eq:delay_func}
			\end{equation}
		...
		\subsubsection{行程时间}
			...
			美国联邦公路局路阻函数(即BPR函数)如\eqref{bpr_plain}所示。
			\begin{equation}
			...
			特点，增加行人对机动车的影响、自行车对机动车的影响。结合已有的研究成果\upcite{李向朋2014城市交通拥堵对策—封闭型小区交通开放研究}，得到行人干扰修正系数如表\ref{tab:walker_noise}。自行车分别对车辆的影响系数，得到改进BPR函数。
			\begin{table}[!htbp]
				\centering
				\caption{行人干扰的修正系数}
				...
				\begin{tabular}{c|cccccc}
					\hline 
					干扰程度 & 很严重 & 严重 & 较严重 & 一般 & 很小 & 无 \\ 
					\hline 
					$\eta$ & 0.5 & 0.6 & 0.7 & 0.8 & 0.9 & 1.0 \\ 
					\hline 
				\end{tabular}
			\end{table}
		\subsubsection{道路条件}
