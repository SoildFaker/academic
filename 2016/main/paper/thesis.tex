\documentclass[fontset=fandol,a4paper,12pt]{ctexart}
\usepackage{geometry}
\usepackage{graphicx}
...
\begin{document}
  ...
  \end{center}
  \zihao{4}{\songti 
  	\quad \quad 本文对小区开放问题产生的影响进行研究，以考察小区开放对路网优化、道路通行能力提升、交通状况改善的效果为主要目标，从道路结构、交通流量、人为因素等角度出发，引入模拟综合评价模型、Breass悖论、路阻函数及交通分配模型对不同影响因素逐一、详细、数学化分析，并建立相应的数学模型，最后利用Vissim软件仿真，对所建立的数学模型真实化，定量分析小区开放对周边道路通行的影响。
  	
  	对于问题一，通过分析小区开放对周边道路产生的影响，建立影响因素与道路、交通条件的关系，进而确定小区开放对周边道路的影响因素，即路网变化、行人等非机动车、新增路口三个因素对周边道路通行产生的影响。在构建评价指标的过程中，通过建立模糊综合评价模型，得到模糊评判矩阵，构建评价指标体系。

	对于问题二，根据评价指标模型中小区开放对周边道路通行的影响因素，考虑车辆通行过程中阻抗函数、交通流量分配及交通需求，建立车辆通行的数学模型，以研究小区开放对周边道路通行的影响。其中阻抗函数考察交通信号灯的延误和加入行人非机动车的干扰因素的BPR函数；交通流量由Breass悖论对范围加以限制；对比分析两种不同流量分配模型，确定应用系统最优分配模型达到最优交通流量分布模式。    

	对于问题三，构建三种不同类型的小区，结合公路等级、一般等级道路路段通行能力，通过Vissim软件对每条路段上的车流量进行仿真，并将仿真结果与改进后的 BPR 公式所得结果比较，判定小区开放影响。

	最后，通过对小区开放影响的研究，就小区开放前后对周边影响程度，结合评价指标，向城市规划和交通管理部门提出小区开放的建议，以实现小区开放后路网结构得以优化、道路通行能力提高、交通状况改善的目的。

  }
  \textbf{关键词：}\ 模糊评价\ BPR函数\ VISSIM仿真\ 交通分配
  \newpage
  \section{问题重述}
  ...
  \section{模型建立}
	...
	\subsection{问题二}
		...
		\subsubsection{延误时间造成的阻抗}
			...
			\begin{figure}[!htbp]
				\centering
				\subfigure[十字交叉口]{\includegraphics[width=0.4\textwidth]{pic/cross.jpg}}
				\subfigure[丁字交叉口]{\includegraphics[width=0.4\textwidth]{pic/T.jpg}}
				\caption{两种不同的交叉路口}
				\label{fig:cross_t}
			\end{figure}
			\begin{figure}[!htbp]
		...
		\subsubsection{改进道路阻抗模型}
			...
			\begin{table}[!htbp]
				...
				\end{tabular}
			\end{table}
			新开放的小区路线一般为双向两车道的城市支路，车流量的构成十分复杂，具有大量行人与非机动车干扰，当非机动车的流量没有超过道路通行能力时，非机动车对机动车的影响系数取0.8，而当非机动车的流量超过道路通行能力时非机动车对车辆的影响系数由\eqref{eq:bike}得到。
			\begin{equation}
				\eta_b = 0.8-\frac{(q_b/Q_b+0.5-W_2)}{W_1}
				\label{eq:bike}
			\end{equation}
			其中$ \eta_b $表示非机动车干扰系数，$ q_b $表示非机动车流量，$ Q_b $表示非机动车道通行能力，$ W_1,W_2 $
			表示非机动车道宽度与机动车道宽度。
		...
		\subsubsection{基于系统最优分配Braess悖论的判别}
			...
			查阅论文可知Pas和Principcipio在其论文中指出了Braess悖论不发生的两种情况\upcite{pas1997braess}，一种是交通需求量低时，如\eqref{eq:Q_low}，另一种时交通需求量高时如\eqref{eq:Q_high}，当交通量符合上述两种情况时，不发生Braess现象，其中$\gamma_n$与$\delta_n$分别为总阻抗函数的常数项系数与一次项系数。
			\begin{equation}
				Q<\frac{2(\gamma_1-\gamma_2)}{3\delta_1+\delta_2}
				\label{eq:Q_low}
			\end{equation}
			\begin{equation}
				Q>\frac{2(\gamma_1-\gamma_2)}{\delta_1-\delta_2}
				\label{eq:Q_high}
			\end{equation}
	\subsection{问题三}
		
		在城市交通中，交通网络通行能力的大小会受到某一路段或某一路口的影响于，一般道路等级高的路段会有更多的容纳能力，根据《城市道路工程设计规范》我国城市公路分级如表\ref{tab:road_type}所示。
		\begin{table}[!htbp]
			\centering
			\caption{公路等级类型表}
			\label{tab:road_type}
			\begin{tabular}{c|ccc}
				\toprule[1pt]
				道路级别&设计车速(km/h)&单向机动车道数（条)&分隔带设置\\
				\hline
